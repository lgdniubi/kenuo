		...
		$("#goodselectButton").click(function(){
			...
			if((cateid == "0" || cateid == "") && (goodsName == "")){
				...
				top.layer.open({
				    ...
				    content: "${ctx}/tag/treeselect?url="+encodeURIComponent("/ec/goods/treeGoodsData?&isReal=0&isAppshow=0&goodsCategory="+cateid+"&goodsName="+goodsName)+"&module=&checked=&extId=&isAll=",
				    btn: ['确定', '关闭']
		    	       ,yes: function(index, layero){ //或者使用btn1
								var tree = layero.find("iframe")[0].contentWindow.tree;//h.find("iframe").contents();
								var ids = [], names = [], nodes = [];
					...
					<form:form id="inputForm" modelAttribute="orders" action="#" method="post" class="form-horizontal">
							<table class="table  table-bordered  table-hovertable-condensed   dataTables-example dataTable no-footer">
								...
								<tr>
									...
									<td><input id="goodsPrice" name="goodsPrice" maxlength="10" class="form-control required" readonly="true"   style="width:150px;height:30px;"/></td>
								</tr>
								<tr>
