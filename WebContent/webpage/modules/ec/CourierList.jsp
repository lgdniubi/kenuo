...
<script type="text/javascript">
	...
	function addGoods(show, close, actionId) {
		...
		if (newDate.getTime() < showDate.getTime()) {
			openDialogView('活动商品列表',
					'${ctx}/ec/action/addActionGoodsList?actionId=' + actionId,
					'900px', '700px');
		}
	...
	function actionEdit(show, close, actionId) {
		...
		if (newDate.getTime() < showDate.getTime()) {
			openDialog('查看抢购活动', '${ctx}/ec/action/form?actionId=' + actionId,
					'800px', '550px');
		}
			...
			<div class="ibox-content">
				<div class="clearfix">
					<form:form id="searchForm" modelAttribute="courier"
						action="${ctx}/ec/courier/list" method="post" class="form-inline">
					...
					<div class="row" style="padding-top: 10px;">
						<div class="col-sm-12">
							<div class="pull-left">
								<shiro:hasPermission name="ec:courier:add">
									<a href="#"
										onclick="openDialog('创建模板', '${ctx}/ec/courier/form','600px','600px')"
										class="btn btn-white btn-sm"><i class="fa fa-plus"></i>创建模板</a>
								</shiro:hasPermission>
								<p></p>
							</div>
				...
				<table id="contentTable"
					...
					<thead>
						<tr>
							...
							<th style="text-align: center;">默认首重</th>
							<th style="text-align: center;">默认价格</th>
							<th style="text-align: center;">默认续重</th>
							<th style="text-align: center;">续重价格</th>
					...
					<tbody style="text-align: center;">
						<c:forEach items="${page.list}" var="courier">
							<tr>
								...
								<td>${courier.startingPoint}</td>
								<td>${courier.courierName}</td>
								<c:if test="${courier.type==1}">
									<td>按重量</td>
								</c:if>
								<c:if test="${courier.type==2}">
									<td>按体积</td>
								</c:if>
								<td>${courier.defaultWeight}</td>
								<td>${courier.defaultPrice}</td>
								<td>${courier.defaultAdd}</td>
								<td>${courier.defaultAddPrice}</td>
								<td><fmt:formatDate value="${courier.createDate}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><shiro:hasPermission name="ec:courier:view">
										<a href="#"
											onclick="openDialogView('查看抢购活动', '${ctx}/ec/courier/listform?id=${action.id}','900px','600px')"
											class="btn btn-info btn-xs"><i class="fa fa-search-plus"></i>查看</a>
									</shiro:hasPermission> <shiro:hasPermission name="ec:courier:edit">
										<a href="#"
											onclick="openDialog('编辑活动', '${ctx}/ec/courier/form?id=${action.id}','900px','600px')"
											class="btn btn-success btn-xs"><i class="fa fa-file"></i>修改</a>
									</shiro:hasPermission> <shiro:hasPermission name="ec:courier:edit">
										<a href="#"
											onclick="openDialogView('红包列表', '${ctx}/ec/courier/Couponlist?id=${action.id}','900px','500px')"
