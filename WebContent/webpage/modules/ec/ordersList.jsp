			 ...
			 <sys:message content="${message}"/>
				...
				<form:form id="searchForm" modelAttribute="orders" action="${ctx}/ec/orders/list" method="post" class="form-inline">
					...
					<div class="form-group">
						...
						<p></p>
						<label>创建类型：</label>
						<form:select path="createlogo"  class="form-control" style="width:185px;">
								<form:option value="0">全部</form:option>
								<form:option value="1">客户端订单</form:option>
								<form:option value="2">后台订单</form:option>
						</form:select>
					...
					<div class="col-sm-12">
						<div class="pull-left">
							<shiro:hasPermission name="ec:orders:add">
								<table:addRow url="${ctx}/ec/orders/createOrder" title="订单添加" width="750px" height="650px"></table:addRow><!-- 增加按钮 -->
							</shiro:hasPermission>
				...
				<table id="contentTable" class="table table-striped table-bordered  table-hover table-condensed  dataTables-example dataTable no-footer">
					...
					<c:forEach items="${page.list}" var="orders">
						<tr>
							...
							<c:if test="${orders.createlogo==1}">
							<td>客户端</td>
							</c:if>
							<c:if test="${orders.createlogo==2}">
							<td>后台</td>
							</c:if>
							...
							<td>
			 					<shiro:hasPermission name="ec:orders:view"> 
			 						<a href="#" onclick="openDialogView('商品详情', '${ctx}/ec/orders/goodslist?orderid=${orders.orderid}','800px','500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 商品详情</a>
			 					</shiro:hasPermission>
			 					<shiro:hasPermission name="ec:orders:view"> 
			 						<a href="#" onclick="openDialogView('查看订单', '${ctx}/ec/orders/orderform?orderid=${orders.orderid}','800px','600px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 订单详情</a>
			 					</shiro:hasPermission>
								<shiro:hasPermission name="ec:orders:edit">
									<c:if test="${orders.orderstatus==-1}">
										<a href="#" onclick="openDialog('编辑订单', '${ctx}/ec/orders/orderform?orderid=${orders.orderid}','800px','600px')"  class="btn btn-success btn-xs" ><i class="fa fa-edit"></i>修改</a>
									</c:if>
									<c:if test="${orders.orderstatus==1}">
										<a href="#" onclick="openDialog('编辑订单', '${ctx}/ec/orders/orderform?orderid=${orders.orderid}','800px','600px')"  class="btn btn-success btn-xs" ><i class="fa fa-edit"></i>修改</a>
									</c:if>
								...
								<shiro:hasPermission name="ec:orders:edit"> 
			 					...
			 					<shiro:hasPermission name="ec:orders:return">
									<c:if test="${orders.orderstatus==1}">
										<a href="#" onclick="openDialog('退货订单录入','${ctx}/ec/orders/returnGoddsform?orderid=${orders.orderid}','550px','600px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i>生成退货订单</a>
									</c:if>
									<c:if test="${orders.orderstatus==2}">
										<a href="#" onclick="openDialog('退货订单录入','${ctx}/ec/orders/returnGoddsform?orderid=${orders.orderid}','550px','600px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i>生成退货订单</a>
									</c:if>
									...
									<c:if test="${orders.orderstatus==-2}">
										<a href="#" style="background:#C0C0C0;color:#FFF" class="btn  btn-xs" ><i class="fa fa-edit"></i>生成退货订单</a>
									</c:if>
									<c:if test="${orders.orderstatus==-1}">
										<a href="#" style="background:#C0C0C0;color:#FFF" class="btn  btn-xs" ><i class="fa fa-edit"></i>生成退货订单</a>
									</c:if>
