...
<script type="text/javascript">
		...
		function addActionGoods(){
			...
			top.layer.open({
			    ...
			    area: ['800px', '550px'],
			    title:"添加商品",
			    content: "${ctx}/ec/orders/addAfterSaleGoods?isNeworder=0",
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
					...
					var actualSpeckeyNum = obj.document.getElementById("actualSpeckeyTimes");	//售后转单订单自定的实际规格次数
					...
					$("<tr> "+
						...
						"<td> "+$(actualSpeckeyNum).val()+"</td><input id='actualSpeckeyNum' name='actualSpeckeyNums' type='hidden' value='"+$(actualSpeckeyNum).val()+"'>"+
	...
	//afterPayment 实际付款
	//debtMoney 欠款
	function delFile(obj,costPrice,orderAmount,spareMoney,afterPayment,debtMoney){
...
<body>
	<div class="ibox-content">
		<div class="clearfix">
			<form:form id="inputForm" modelAttribute="orders" action="${ctx}/ec/orders/saveAfterSaleVirtualOrder" method="post" class="form-horizontal">
				<label><font color="red">*</font>手机号码：</label>
				<input id="mobile" id="mobile" maxlength="11" class="form-control required" readonly="readonly" value="${orders.mobile}" style="width:200px"/>&nbsp;&nbsp;
				<label><font color="red">*</font>昵&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;称：</label>
				<input id="username" name="username" type="text" class="form-control required" readonly="readonly" value="${orders.username}" style="width:200px" />
				<input type="hidden" name="userid" id="userid" value="${orders.userid}"/>
				<input type="hidden" name="returnedId" id="returnedId" value="${orders.returnedId}"/>
				<p></p>
				<label><font color="red">*</font>订单性质：</label>
				<form:select path="distinction"  class="form-control" style="width:200px">
					<form:option value="4">售后转单</form:option>
				</form:select>&nbsp;&nbsp;
					...
					<table id="contentTable" class="table table-bordered table-condensed  dataTables-example dataTable no-footer">
						<thead>
							<tr>
								...
								<th style="text-align: center;">实际规格次数</th>
