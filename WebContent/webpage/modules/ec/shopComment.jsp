	...
	<script type="text/javascript">
		...
		function tijiao(){
			if($("#contents").val()!=''){
				...
				$.ajax({
					 type : 'POST',
					 url : '${ctx}/ec/mtmycomment/replyShopComment',
					 data:{'parentId':$("#parentId").val(),'contents':$('#contents').val(),'isShow':$('input:radio:checked').val(),'shopId':$('#shopId').val()},
	...
	<div class="wrapper wrapper-content">
		<div class="ibox">
	    	...
	    	<div class="ibox-content">
	    		<div class="clearfix">
	    			<form:form id="searchForm" modelAttribute="comment" action="${ctx}/ec/mtmycomment/shopComment" method="post" class="form-inline">
						...
						<div class="form-group">
							...
							<label>选择店铺：</label>
							<sys:treeselect id="office" name="office.id" value="${comment.office.id}" 
									labelName="office.name" labelValue="${comment.office.name}" 
						     		title="店铺" url="/sys/office/newTreeData" cssClass="form-control required" allowClear="true" notAllowSelectParent="true"/>
							<label>评论时间：</label>
					...
					<tbody style="text-align: center;">
						<c:forEach items="${page.list }" var="comment">
							<tr>
							    ...
							    <td>
							     	<button class="btn btn-info btn-xs" onclick="look('${comment.commentId }','${comment.shopId }')"><i class="fa fa-search-plus"></i> 查看详细</button>
							    </td>
							</tr>
						</c:forEach>
					</tbody>
