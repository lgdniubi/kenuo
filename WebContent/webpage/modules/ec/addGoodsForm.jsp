...
<script type="text/javascript">
	...
	function findGood(){
		...
		var newGoodsIds = $("#newGoodsIds").val();
		var arr = new Array(); //数组定义标准形式，不要写成Array arr = new Array();
	    var all = new Array(); //定义变量全部保存
		$("#select1").empty();	
		$("#select2 option").each(function (){
		 ...
		 if(newValidateForm.form()){
			$.ajax({
				 type:"get",
				 dataType:"json",
				 url:"${ctx}/ec/goods/queryGoods?goodsCategory="+cateid+"&goodsName="+goodsName+"&goodsIds="+newGoodsIds+"&cityIds="+$("#cityIds").val(),
				 success:function(date){
					var data=date;
					if(data.length < 1){
						top.layer.alert('商品不存在或商品已参加该城市的异价!', {icon: 0, title:'警告'});
					}
					if(arr.length>0){
			...
			<div class="ibox-content">
				<div class="clearfix">
					<form:form id="inputForm" modelAttribute="goods" action="" method="post" class="form-horizontal">
						<input id="cityIds" name="cityIds" value="${cityIds}" type="hidden">
						<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
							...
							<tr>
								<td>
									<label class="pull-right" >商品Ids：</label>
								</td>
								<td>
									<textarea rows="7" id="newGoodsIds" name="newGoodsIds" class="form-control"></textarea>
									<p></p>
