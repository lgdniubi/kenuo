...
// 				});
				if($("#actionId").val() > 0){
					//可见范围赋值
					var checkBox = $("#isOpen_").val();
					if(checkBox == "0,"){
				    	$("input[name='isOpen'][id='isOpen']").attr("disabled", false);//公开:可选
						$("input[name='isOpen'][id='isNotOpen']").attr("disabled", true);//商家:禁选
				    }else if(checkBox.length != 0){
				    	$("input[name='isOpen'][id='isOpen']").attr("disabled", true);//公开:禁选
						$("input[name='isOpen'][id='isNotOpen']").attr("disabled", false);//商家:可选
				    }
			...
			function selectOpen(o){
				...
				if(o == 0){
					...
					obj = document.getElementsByName("isOpen");
				    check_val = [];
				    for(k in obj){
				        if(obj[k].checked)
				...
				<div class="clearfix">
					<form:form id="inputForm" modelAttribute="actionInfo" action="${ctx}/ec/action/save" method="post" class="form-horizontal">
						...
						<form:select path="level" class="form-control required" style="width:200px;" >
 						...
 						<p></p>
						<label>可见范围：&nbsp;</label>
 						<label><input type="checkbox" id="isOpen" name="isOpen" class="onoffswitch-checkbox" value="0" onclick="selectOpen(0)">公开</label>
 						<c:forEach items="${franchiseeList}" var="franchisee">
 							<label><input type="checkbox" id="isNotOpen" name="isOpen" class="onoffswitch-checkbox" value="${franchisee.id}" onclick="selectOpen(1)">${franchisee.name}</label>
 						</c:forEach>
 						<input type="hidden" id="isOpen_" name="isOpen_" class="onoffswitch-checkbox" value="${actionInfo.isOpen}">
 						<p></p>
 						<label>排&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;序：</label>
