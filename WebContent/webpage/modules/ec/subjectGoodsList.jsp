...
<script type="text/javascript">
	...
	function  deleteGoods(goodsId,subId){
		if(confirm("确认要删除吗？","提示框")){
			newDelete(goodsId,subId);			
		}
	}
		
	function newDelete(goodsId,subId){
		$.ajax({
			type:"post",
			url:"${ctx}/ec/subject/delGoods?goodsId="+goodsId+"&subId="+subId,
			success:function(date){
				if(date=="success"){
					top.layer.alert('删除成功!', {icon: 1, title:'提醒'});
					window.location="${ctx}/ec/subject/subjectGoodsList?subId="+subId;
				}
				if(date=="error"){
	...
	function addGoods(id,flag){
		top.layer.open({
		    ...
		    area: ['900px', '550px'],
		    title:"添加商品",
		    content: "${ctx}/ec/subject/subjectGoodsForm?subId="+id+"&flag="+flag,
		    btn: ['确定', '关闭'],
		    yes: function(index, layero){
		     ...
		     /*     $(select2).each(function () {
				...
				$.ajax({
					type:"post",
					url:"${ctx}/ec/subject/saveSubjectGoods?goodsIds="+arr+"&subId="+id,
					success:function(date){
						if(date=="success"){
							top.layer.alert('保存成功!', {icon: 1, title:'提醒'});
							window.location="${ctx}/ec/subject/subjectGoodsList?subId="+id;
						}
						if(date=="error"){
	...
	<div class="wrapper wrapper-content">
		<div class="ibox">
			<div class="ibox-content">
				<div class="clearfix">
				<form id="searchForm" action="${ctx}/ec/subject/subjectGoodsList" method="post" class="navbar-form navbar-left searcharea">
					<!-- 翻页隐藏文本框 -->
				...
				<div>
					<a href="#" onclick="addGoods(${subject.subId},'add')" class="btn btn-primary btn-xs" ><i class="fa fa-plus"></i>添加商品</a>
				</div>
				<p></p>
						...
						<c:forEach items="${page.list}" var="page">
							<tr>
								...
								<td style="text-align: center;">
									<shiro:hasPermission name="ec:subject:deleteGoods">
										<a href="#" onclick="deleteGoods(${page.goodsId},${subject.subId})" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i>删除</a>
									</shiro:hasPermission> 
								</td>
							</tr>
						</c:forEach>
