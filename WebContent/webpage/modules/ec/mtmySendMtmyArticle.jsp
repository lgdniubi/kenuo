...
<div class="wrapper-content">
    <div class="wrapper wrapper-content">
        <div class="row">
            ...
            <div class="mail-box">
                 <div class="mail-body">
					<form:form id="inputForm" modelAttribute="mtmyArticle" action="${ctx}/ec/mtmyArticleList/save" method="post" class="form-horizontal">
						<input type="hidden" id="id" name="id" value="${mtmyArticle.id}">
		                <input type="hidden" id="contents" name="contents" value="${mtmyArticle.contents}"><!-- 内容 -->
                       <div class="form-group">
                            <label class="col-sm-2 control-label"><font color="red">*</font>标题：</label>
                           <div class="col-sm-8">
                               <input type="text" id="title" maxlength="200" name="title"  class="form-control" value="${mtmyArticle.title}">
						   </div>
					   </div>
					   <div class="form-group">
                            <label class="col-sm-2 control-label"><font color="red">*</font>短标题：</label>
                           <div class="col-sm-8">
                               <input type="text" id="shortTitle" maxlength="100" name="shortTitle"  class="form-control" value="${mtmyArticle.shortTitle}">
						   </div>
					   </div>
					   <div class="form-group">
                           <label class="col-sm-2 control-label"><font color="red">*</font>作者：</label>
                           <div class="col-sm-8">
                               <input type="text" id="authorId" maxlength="50" name="authorId"  class="form-control" value="${mtmyArticle.authorId}">
						   </div>
					   </div>
					   <div class="form-group">
	                          ...
	                          <select class="form-control" id="type" name="type">
								   ...
								   <c:forEach items="${fns:getDictList('mtmyArticle_type')}" var="mtmyArticle_type">
									    <c:choose>
											<c:when test="${mtmyArticle_type.value eq mtmyArticle.type}">
												<option value="${mtmyArticle_type.value }" selected="selected">${mtmyArticle_type.label }</option>
											</c:when>
											<c:otherwise>
												<option value="${mtmyArticle_type.value }">${mtmyArticle_type.label }</option>
											</c:otherwise>
										</c:choose>
									</c:forEach>
							  </select>
					   ...
					   <div class="form-group">
                           <label class="col-sm-2 control-label"><font color="red">*</font>首图：</label>
                           <div class="col-sm-8">
                           		<input type="file" name="file_photo_upload" id="file_photo_upload">
                           		<input type="hidden" id="firstImage" name="firstImage" value="${mtmyArticle.firstImage}" class="form-control" style="width: 350px;" >   
								<div id="file_photo_queue" style="margin top:10px;"></div> 
								<img id="firstImageSrc" src="${mtmyArticle.firstImage}" alt="" style="width: 200px;height: 100px;"/>
						   </div>
					   </div>
					   <div class="form-group">
                           <label class="col-sm-2 control-label"><font color="red">*</font>摘要：</label>
                           <div class="col-sm-8">
                               <input type="text" id="digest" name="digest"  class="form-control" value="${mtmyArticle.digest}">
						   </div>
					   </div>
					   <div class="form-group">
	             ...
	             <div class="mail-body text-right tooltip-demo">
           				<button type="button" class="btn btn-primary  btn-sm" onclick="sendLetter()"><i class="fa fa-reply"></i>提交审核</button>
    ...
    <script>
		...
		$(document).ready(function() {
			...
			$("#file_photo_upload").uploadify({
				...
				'onUploadSuccess' : function(file, data, response) {
					var jsonData = $.parseJSON(data);//text 转 json
					if (jsonData.result == '200') {
						$("#firstImage").val(jsonData.file_url);
						$('.firstImage').attr('href', jsonData.file_url);
						$("#firstImageSrc").attr('src', jsonData.file_url);
					}
				}
			});
		...
		function sendLetter(){
			...
			}else if($("#shortTitle").val() == ""){
				top.layer.alert('短标题不能为空！', {icon: 0});
			}else if($("#authorId").val() == ""){
				top.layer.alert('作者不能为空！', {icon: 0});
			}else if($("#type").val() == -1){
				top.layer.alert('类型不能为空！', {icon: 0});
			}else if($("#categoryId").val() == ""){
				top.layer.alert('分类不能为空！', {icon: 0});
			}else if($("#firstImage").val() == ""){
				top.layer.alert('首图不能为空！', {icon: 0});
			}else if($("#digest").val() == ""){
				top.layer.alert('摘要不能为空！', {icon: 0});
