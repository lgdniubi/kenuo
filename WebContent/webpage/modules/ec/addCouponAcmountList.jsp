	...
	<div class="wrapper wrapper-content">
		<div class="ibox">
			<div class="ibox-content">
				...
				<div>
					<c:if test="${couponAcmountMapping.dateCompare>0}">
						<a href="#" style="background: #C0C0C0; color: #FFF" class="btn  btn-xs"><i class="fa fa-plus"></i>添加面值</a>
					</c:if>
					<c:if test="${couponAcmountMapping.dateCompare<0}">
						<a href="#" onclick="addActionGoods(0)" class="btn btn-primary btn-xs" ><i class="fa fa-plus"></i>添加面值</a>
					</c:if>
					...
					<tbody style="text-align: center;">
						<c:forEach items="${list}" var="list">
							<tr>
								...
								<td>
								<c:if test="${list.status==1}">
									<a href="#" onclick="updateStatus('${list.amountId}','2')" class="btn btn-primary btn-xs" ><i class="fa fa-file"></i>开启</a>
								</c:if>
								<c:if test="${list.status==2}">
									<a href="#" onclick="updateStatus('${list.amountId}','1')" class="btn btn-danger btn-xs" ><i class="fa fa-close"></i>关闭</a>
								</c:if>	
								<c:if test="${list.status==1}">
	                                ...
	                                <c:if test="${list.totalNumber==list.couponNumber}">
                                	<a href="#" onclick="addActionGoods(${list.amountId})" class="btn btn-success btn-xs"><i class="fa fa-edit"></i>修改</a>
                                </c:if>	
	                            </c:if>
	                            <c:if test="${list.status==2}">
	                            <a href="#" style="background: #C0C0C0; color: #FFF" class="btn  btn-xs"><i class="fa fa-edit"></i>修改</a>
