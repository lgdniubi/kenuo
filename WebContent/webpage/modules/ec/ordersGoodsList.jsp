		...
		<div class="ibox">
				...
				<div class="clearfix">
				 <div class="pull-left">
					...
					<table id="contentTable" class="table table-striped table-bordered  table-condensed dataTables-example dataTable">
						<thead>
							<tr>
					...
					<div class=" pull-left">
						<hr>
						<h4>使用优惠明细</h4>
						<table id="coupTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable" >
						<thead>
							<tr>
								...
								<c:forEach items="${couplist}" var="couplist">
									<tr>
										...
										<td style="text-align: center;">
											...
											<c:if test="${couplist.type==2}">
												活动页
											</c:if>
		
											<c:if test="${couplist.type==3}">
												新注册
											</c:if>
											<c:if test="${couplist.type==4}"> 
												手工红包
											</c:if>
										</td>
										<td style="text-align: center;">
											<c:if test="${couplist.usedType==1}">
												全部商品
											</c:if>
											<c:if test="${couplist.usedType==2}">
												指定分类
											</c:if>
											<c:if test="${couplist.usedType==3}">
												指定商品
											</c:if>
										</td>
										...
										<td style="text-align: center;">
											<a href="#" onclick="openDialogView('信息列表', '${ctx}/ec/activity/CateGoodsList?id=${couplist.counponId}&usedType=${couplist.usedType}','300px','400px')"
