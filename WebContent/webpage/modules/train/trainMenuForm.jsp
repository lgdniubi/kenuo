...
<body>
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/train/menu/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		   ...
		   <tbody>
		      <tr>
		         ...
		         <td class="width-35" ><sys:treeselect id="menu" name="parent.id" value="${menu.parent.id}" labelName="parent.name" labelValue="${menu.parent.name}"
					title="菜单" url="/train/menu/treeData" extId="${menu.id}" cssClass="form-control required"/></td>
		      ...
		      <tr>
		         ...
		         <td  class="width-35" >
					<form:select path="isPay" cssClass="form-control required">
						<form:option value="0">免费</form:option>
						<form:option value="1">收费</form:option>
					</form:select></td>
<%-- 		         <form:radiobuttons path="isPay"  itemLabel="label" itemValue="value" htmlEscape="false" class="required i-checks "/><form:input path="isPay" htmlEscape="false" maxlength="10" class="form-control "/> --%>
<!-- 					<span class="help-inline">链接打开的目标窗口，默认：mainFrame</span> -->
		      </tr>
		      <tr>
		         <td  class="width-15 active"><label class="pull-right">图标:</label></td>
		         <td class="width-35" ><input type="text" id="icon" name="icon" value="${menu.icon}" class="form-control "></td>
		      ...
		      <tr>
		         ...
		         <td class="width-35" >
		         	<form:radiobutton path="isShow" class="required i-checks " value="0"/> 显示
		         	<form:radiobutton path="isShow" class="required i-checks " value="1"/> 隐藏
