				...
				rules:{ 
					name:{
	                    remote:{
							...
							url: "${ctx}/train/mdmenu/checkName?oldName=${menu.name}&parentId=${menu.parent.id}",
							data: {                     //要传递的数据
								name: function() {
						            return $("#name").val();
						        }
...
<body>
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/train/mdmenu/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		   ...
		   <tbody>
		      <tr>
		         ...
		         <td class="width-35" ><sys:treeselect id="menu" name="parent.id" value="${menu.parent.id}" labelName="parent.name" labelValue="${menu.parent.name}"
					title="菜单" url="/train/mdmenu/treeData" extId="${menu.id}" cssClass="form-control required"/></td>
