								...
								<td style="text-align: center;">
									...
									<c:if test="${refund.orderType eq '2'}">
										<c:if test="${refund.orderStatus eq '2'}">
											<a href="${ctx}/train/refundOrder/makeSureInAccount?office_id=${refund.arrearageOffice}&order_id=${refund.orderId}"  onclick="return confirmx('确定已入账？', this.href)" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i>确认入账</a>
										</c:if>
									</c:if>
									</shiro:hasPermission>
