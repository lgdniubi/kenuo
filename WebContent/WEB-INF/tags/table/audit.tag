...
$(document).ready(function() {
    $('#${id} thead tr th input.i-checks').on('ifChecked', function(event){ //ifCreated 事件应该在插件初始化之前绑定 
    	  $('#${id} tbody tr td input.i-checks').not(':disabled').iCheck('check');
    	});

    $('#${id} thead tr th input.i-checks').on('ifUnchecked', function(event){ //ifCreated 事件应该在插件初始化之前绑定 
    	  $('#${id} tbody tr td input.i-checks').not(':disabled').iCheck('uncheck');
    	});
    
});

	function auditAll(obj){
		...
		// var url = $(this).attr('data-url');
		  ...
		  if(ids == ""){
			...
			top.layer.confirm('确认批量通过?', {icon: 3, title:'系统提示'}, function(index){
				var url = "${url}?ids="+ids;
				console.log(ids);
				console.log(url);
				if('${isSelf}')
