...
public class OrderTimeOut extends CommonService{
	...
	/**
	 * 2017年5月17日00:35:24
	 * 由于520活动 与 普通抢购活动 取消订单冲突 新增定时器
	 * coffee
	 */
	public void orderTimeOut1(){
		logger.info("[work0],start,过期订单设置,开始时间："+df.format(new Date()));
		...
		startDate = new Date();
		taskLog.setJobName("orderTimeOut1");
		taskLog.setStartDate(startDate);
		
		String now_order = null;//当前订单号
		
		try {
			int timeout = Integer.parseInt(ParametersFactory.getMtmyParamValues("order_timeout1"));
			
			logger.info("[过期普通订单]，订单过期时间为："+timeout);
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("minute", timeout);
			map.put("order_type", 0);//普通订单
			List<Orders> list = ordersService.queryNotPayOrder1(map);
