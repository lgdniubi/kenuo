 ...
 */
@Component
public class ReportShopGoods extends CommonService{

	private Logger logger = Logger.getLogger(ReportShopGoods.class);
	 ...
	 * 店铺商品统计
	 */
	public void reportShopGoods(){
		logger.info("[work0],start,店铺商品统计，开始时间："+df.format(new Date()));
		...
		startDate = new Date();
		taskLog.setJobName("reportShopGoods");
		taskLog.setStartDate(startDate);
		
		try {
			shopReportService.reportShopGoods("reportShopGoods");
			taskLog.setJobDescription("[work],店铺商品统计");
			taskLog.setStatus(0);//任务状态
		} catch (Exception e) {
			logger.error("#####【定时任务reportShopGoods】店铺商品统计,出现异常，异常信息为："+e.getMessage());
			taskLog.setStatus(1);
		...
		logger.info("[work0],end,店铺商品统计,结束时间："+df.format(new Date()));
	}
}
