 ...
 */
@Component
public class ReportBeauticianAchievement extends CommonService{

	private Logger logger = Logger.getLogger(ReportBeauticianAchievement.class);
	 ...
	 * 美容师绩效统计
	 */
	public void reportBeauticianAchievement(){
		logger.info("[work0],start,美容师绩效统计，开始时间："+df.format(new Date()));
		...
		startDate = new Date();
		taskLog.setJobName("reportBeauticianAchievement");
		taskLog.setStartDate(startDate);
		
		try {
			shopReportService.reportBeauticianAchievement("reportBeauticianAchievement");
			taskLog.setJobDescription("[work],美容师绩效统计");
			taskLog.setStatus(0);//任务状态
		} catch (Exception e) {
			logger.error("#####【定时任务reportBeauticianAchievement】美容师绩效统计,出现异常，异常信息为："+e.getMessage());
			taskLog.setStatus(1);
		...
		logger.info("[work0],end,美容师绩效统计,结束时间："+df.format(new Date()));
	}
}
