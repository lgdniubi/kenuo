...
public class TrainMenuService extends BaseService implements InitializingBean {
	...
	public User getUserByLoginName(String loginName) {
		return TrainUserUtils.getByLoginName(loginName);
	}
	...
	public List<User> findUserByOfficeId(String officeId) {
		List<User> list = (List<User>) CacheUtils.get(TrainUserUtils.USER_CACHE,
				TrainUserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + officeId);
		if (list == null) {
			User user = new User();
			user.setOffice(new Office(officeId));
			list = userDao.findUserByOfficeId(user);
			CacheUtils.put(TrainUserUtils.USER_CACHE, TrainUserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + officeId, list);
		}
		return list;
	}
	/**
	...
	public void saveUser(User user) {
		...
		User currentUser = TrainUserUtils.getUser();//记录图片上传时,获取当前登录用户的信息
...
//			saveUserLog1(user);
			...
			if (oldUser.getOffice() != null && oldUser.getOffice().getId() != null) {
				CacheUtils.remove(TrainUserUtils.USER_CACHE,
						TrainUserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + oldUser.getOffice().getId());
			}
...
//			用户商家发生变化时,数据权限跟着变化
		...
		if (StringUtils.isNotBlank(user.getId())) {
			...
			// 清除用户缓存
			TrainUserUtils.clearCache(user);
			// // 清除权限缓存
	...
	public void updateUserLoginInfo(User user) {
		...
		// 更新本次登录信息
		user.setLoginIp(TrainUserUtils.getSession().getHost());
	...
	public List<Role> findAllRole() {
		return TrainUserUtils.getRoleList();
	}
	...
	public void saveRole(Role role) {
		...
		// 清除用户角色缓存
		TrainUserUtils.removeCache(TrainUserUtils.CACHE_ROLE_LIST);
		// // 清除权限缓存
		// systemRealm.clearAllCachedAuthorizationInfo();
	}
	...
	public PCMenu getMenu(String id) {
		return trainMenuDao.get(id);
	}

	public List<PCMenu> findAllMenu() {
		return TrainUserUtils.getMenuList();
	}

	@Transactional(readOnly = false)
	public void saveMenu(PCMenu menu) {
		...
		if (StringUtils.isBlank(menu.getId())) {
			menu.preInsert();
			trainMenuDao.insert(menu);
		} else {
			menu.preUpdate();
			trainMenuDao.update(menu);
		}

		// 更新子节点 parentIds
		PCMenu m = new PCMenu();
		m.setParentIds("%," + menu.getId() + ",%");
		List<PCMenu> list = trainMenuDao.findByParentIdsLike(m);
		for (PCMenu e : list) {
			e.setParentIds(e.getParentIds().replace(oldParentIds, menu.getParentIds()));
			trainMenuDao.updateParentIds(e);
		}
		// 清除用户菜单缓存
	...
	@Transactional(readOnly = false)
	public void updateMenuSort(PCMenu menu) {
		trainMenuDao.updateSort(menu);
		// 清除用户菜单缓存
		TrainUserUtils.removeCache(TrainUserUtils.CACHE_MENU_LIST);
		// // 清除权限缓存
		// systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void deleteMenu(PCMenu menu) {
		trainMenuDao.delete(menu);
		// 清除用户菜单缓存
		TrainUserUtils.removeCache(TrainUserUtils.CACHE_MENU_LIST);
		// // 清除权限缓存
		// systemRealm.clearAllCachedAuthorizationInfo();
