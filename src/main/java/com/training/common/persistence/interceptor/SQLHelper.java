...
public class SQLHelper {
    ...
    public static int getCount(final String sql, final Connection connection,
    	...
    	/*
    	 * 优化：分页查询时，把order by 的语句切割掉，加快查询总数内容
    	 * 时间：2016年8月24日15:00:47
    	 */
    	final String countSql = "select count(1) from (" + removeHqlOrderby(sql) + ") tmp_count";
    ...
    /**
     * 去除hql语句后order by 结尾的语句
     * @param hqlString
     * @return
     */
	private static String removeHqlOrderby(String hqlString){
    	int lastIndex = hqlString.toUpperCase().lastIndexOf("ORDER BY");
    	if(lastIndex > 1){
    		hqlString = hqlString.substring(0, lastIndex);
    	}
    	return hqlString;
    }
