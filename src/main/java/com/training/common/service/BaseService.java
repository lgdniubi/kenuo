	...
	public static String dataScopeFilter(User user, String officeAlias) {
		...
		if(!user.isAdmin()){
			...
			if(User.DATA_SCOPE_OFFICE_AND_CHILD.equals(String.valueOf(user.getDataScope()))){
				...
				sqlString.append(" AND( "+officeAlias+".id = '" + user.getOffice().getId() + "'");
				...
				//数据范围(2:按明细设置)
				sqlString.append(" LEFT JOIN trains.sys_user_office o2 ON o2.office_id = "+officeAlias+".id ");
				sqlString.append(" WHERE o2.user_id = '"+user.getId()+"'");
			}
		}else{
			//当用户为“管理员”,查询所有数据
			sqlString.append(" WHERE 1 = 1 ");
//			sqlString.append(" AND "+officeAlias+".grade = 2 ");     //咖啡注释   2016年11月15日09:47:59 
		}
		return sqlString.toString();
	}
	/**
	 * 用于超级管理员  查看机构管理 
	 * 咖啡  2016年11月16日15:46:18
	 * @param officeAlias
	 * @return
	 */
	public static String superDataScope(String officeAlias){
		StringBuilder sqlString = new StringBuilder();
		sqlString.append(" WHERE 1 = 1 ");
